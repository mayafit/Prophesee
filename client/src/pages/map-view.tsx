...
import {
  MapToolsPanel,
  type MapTool,
} from "@/components/toolbar/map-tools-panel";
...
export default function MapView() {
  ...
  const getStableQueryKey = () => {
    ...
    return [
      "/api/sar-images",
      {
        ...
        suppliers: suppliers ? JSON.stringify(suppliers) : undefined,
      },
    ];
  };
  ...
  const {
    ...
    queryFn: async () => {
      ...
      addLogEntry(
        `Executing search with parameters: ${JSON.stringify(searchParams)}`,
        "info",
      );
      ...
      try {
        ...
        addLogEntry(
          `Found ${data.length} images matching search criteria`,
          "success",
        );
        return data;
      } catch (err) {
  ...
  return (
    <div className="h-screen w-full relative bg-background">
      ...
      {/* Tool Panels */}
      {activeTool === "logs" && (
        <ToolPanel title="System Logs" onClose={() => setActiveTool(null)}>
          <LogPanel entries={logEntries} onClear={() => setLogEntries([])} />
        </ToolPanel>
      )}

      {activeTool === "search" && (
        <ToolPanel title="Search" onClose={() => setActiveTool(null)}>
      ...
      {activeTool === "layers" && (
        <ToolPanel title="Layers" onClose={() => setActiveTool(null)}>
              ...
              <div className="pt-2 border-t mt-2">
                ...
                <div className="space-y-1 max-h-[300px] overflow-auto pr-1">
                  {activeLayers.map((layer) => (
                    <div
                      key={layer.id}
                      className="flex items-center justify-between bg-muted/50 rounded-sm p-1"
                    >
      ...
      {activeTool === "wms" && (
        <ToolPanel title="WMS Suppliers" onClose={() => setActiveTool(null)}>
          <WmsSupplierPanel
            onSearchWmsLayers={(supplier, layerName) => {
              ...
              addLogEntry(
                `Searching for imagery from supplier: ${supplier}, layer: ${layerName}`,
                "info",
              );
      ...
      {activeTool === "info" && (
        <ToolPanel title="Information" onClose={() => setActiveTool(null)}>
          <InfoPanel />
        </ToolPanel>
      )}

      {activeTool === "settings" && (
        <ToolPanel title="Settings" onClose={() => setActiveTool(null)}>
          <SettingsPanel
            baseLayer={baseLayer}
